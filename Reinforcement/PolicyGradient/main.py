...
from params import RESULT_PATH, SAVED_MODEL_PATH, SEQUENCE, get_args
...
def train(cfg):
    ...
    log_dir = os.path.split(os.path.abspath(__file__))[
        0]+"/logs/train/" + SEQUENCE
    ...
    for i_episode in range(cfg.train_eps):
        ...
        writer.add_scalars('rewards', {
                           'raw': rewards[-1], 'moving_average': moving_average_rewards[-1]}, i_episode+1)
    writer.close()
    print('Complete training！')
    save_model(agent, model_path=SAVED_MODEL_PATH)
    '''存储reward等相关结果'''
    save_results(rewards, moving_average_rewards,
                 tag='train', result_path=RESULT_PATH)
    plot(rewards)
...
if __name__ == "__main__":
    ...
    if cfg.train:
        ...
        model_path = os.path.split(os.path.abspath(__file__))[
            0]+"/saved_model/"
