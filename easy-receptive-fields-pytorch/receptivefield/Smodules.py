...
class DeformConv2D(nn.Module):
    ...
    def forward(self, x, offset):
        ...
        offsets_index = Variable(torch.cat([torch.arange(
            0, 2*N, 2), torch.arange(1, 2*N+1, 2)]), requires_grad=False).type_as(x).long()
...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, stride, step, plugin):
