...
parser.add_argument('--train_portion', default=0.5, type=float, help='train portion')
parser.add_argument('--lr', default=0.1, type=float, metavar='LR', help='initial learning rate')
parser.add_argument('--min_lr', default=0.0, type=float, metavar='LR', help='initial learning rate')
parser.add_argument('--momentum', default=0.9, type=float, metavar='M', help='momentum')
parser.add_argument('--weight_decay', default=5e-4, type=float, metavar='W', help='weight decay (default: 5e-4)')
parser.add_argument('--arch_lr', default=3e-4, type=float, metavar='ARCH-LR', help='initial arch learning rate')
parser.add_argument('--arch_weight_decay', default=1e-3, type=float, metavar='ARCH-WD', help='arch weight decay (default: 5e-4)')
parser.add_argument('--drop_path_rate', default=0., type=float, metavar='ARCH-LR', help='initial arch learning rate')
parser.add_argument('--linear_dp_rate', action='store_true', help='linearly increase dp rate')
parser.add_argument('--dropout', default=0., type=float, metavar='ARCH-LR', help='initial arch learning rate')
...
parser.add_argument(
    ...
    type=str,
    help=
    'convbn forward with different mask: mix_channel or random_mix_channel or sample_channel or sample_random_channel or sample_sepmask_channel or sample_sepproject_channel or sample_localfree_channel or sample_localsepmask_channel or sample_localsepadd_channel'
)
parser.add_argument('--alpha_type', default='sample_trackarch', type=str, help='how to cal alpha in forward process: mix, sample_uniform, sample_fair, sample_flops_uniform, sample_flops_fair, sample_sandwich, sample_trackarch')
...
parser.add_argument('--sandwich_N', type=int, default=2, help='used in sample_sandwich')
...
parser.add_argument('--track_file', default='Track1_200_archs.json', type=str, help='used in sample_trackarch')
parser.add_argument('--sample_accumulation_steps', type=int, default=3, help='used in sample_based method')
...
parser.add_argument('--cutout_lenth', type=int, default=16, help='cutout length')
parser.add_argument('--distill', action='store_true', help='distill')
parser.add_argument('--distill_lamda', type=float, default=1.0, help='distill lamda')
parser.add_argument('--min_distill', action='store_true', help='min distill')
parser.add_argument('--min_distill_lamda', type=float, default=1.0, help='min distill lamda')
parser.add_argument('--tauloss', action='store_true', help='tauloss')
parser.add_argument('--tauloss_lamda', type=float, default=1.0, help='tauloss lamda')
parser.add_argument('--tauloss_noise', type=float, default=1e-2, help='tauloss noise')
parser.add_argument('--warmup', action='store_true', help='warmup')
parser.add_argument('--warmup_step', type=int, default=2000, help='warmup step')
...
parser.add_argument('--save_alpha', action='store_true', help='save alpha of all epoch')
...
def main():
    ...
    if 'mix' == args.alpha_type:
        arch_optimizer = torch.optim.Adam(model.alpha, lr=args.arch_lr, betas=(0.5, 0.999), weight_decay=args.arch_weight_decay)
    else:
        arch_optimizer = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.linear_dp_rate and args.drop_path_rate > 0.:
            model.set_drop_path_rate(args.drop_path_rate * (epoch - args.start_epoch) / (args.epochs - args.start_epoch))

        train(train_queue, valid_queue if 'mix' == args.alpha_type else None, model, criterion if args.label_smooth == 0 else criterion_smooth, soft_criterion, optimizer, arch_optimizer, lr_scheduler, warmup_scheduler, epoch, args)
    ...
    if 'mix' == args.alpha_type:
        alpha1, alpha2, alpha3 = model.alpha_cal()
        generate_result(os.path.join(args.save_dir, 'result.json'), alpha1, alpha2, alpha3)
...
def train(train_queue, valid_queue, model, criterion, soft_criterion, optimizer, arch_optimizer, scheduler, warmup_scheduler, epoch, args):
    ...
    for i, (input, target) in enumerate(train_queue):
        ...
        if 'mix' == args.alpha_type:
            ...
            loss.backward()
            soft_target_var = torch.nn.functional.softmax(output, dim=1).detach()
            ...
            for _ in range(args.sample_accumulation_steps):
                ...
                new_input_var = input_var + args.tauloss_noise * 2 * (torch.rand(input_var.size()) - 1).cuda()
                model.alpha_pop()
                ...
                loss = 0.25 * (loss1 + loss2 + loss3 + loss4) + 0.5 * args.tauloss_lamda * max(0, -torch.sign(loss1 - loss2) * (loss3 - loss4) - torch.sign(loss3 - loss4) * (loss1 - loss2))
                loss.backward()  # compute gradient
            optimizer.step()  # do SGD step
        else:
            optimizer.zero_grad()  # zero gradient
            for _ in range(args.sample_accumulation_steps):
                ...
                if args.distill:
                    ...
                    loss = 0.5 * (loss + teacher_loss) + args.distill_lamda * distill_loss
                    if args.min_distill:
                        min_output = model(input_var, [4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4])
                        min_loss = criterion(min_output, target_var)
                        min_distill_loss = soft_criterion(min_output, soft_target_var)
...
def get_data_loader(args):
    ...
    if 'mix' == args.alpha_type:
        ...
        train_queue = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[:split]), pin_memory=True, num_workers=args.workers)
        valid_queue = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[split:num_train]), pin_memory=True, num_workers=args.workers)
    else:
        valid_data = datasets.CIFAR100(root='./data', train=False, transform=transforms.Compose([transforms.ToTensor(), normalize]), download=True)
        train_queue = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, pin_memory=True, shuffle=True, num_workers=args.workers)
        valid_queue = torch.utils.data.DataLoader(valid_data, batch_size=args.batch_size, pin_memory=True, shuffle=True, num_workers=args.workers)
