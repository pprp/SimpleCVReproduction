...
class Bottleneck(nn.Layer):
    def __init__(self, len_list, stride=1, downsampling=False):
        ...
        self.conv1 = self.Conv(self.len_list[IND-1], self.len_list[IND], kernel_size=1, stride=1, padding=0)
        self.conv2 = self.Conv(self.len_list[IND], self.len_list[IND+1], kernel_size=3, stride=stride, padding=1)
        self.conv2 = self.Conv(self.len_list[IND+1], self.len_list[IND+2], kernel_size=1, stride=1, padding=0)


        if self.downsampling or (self.len_list[IND-1] != self.len_list[IND+2]):
            self.downsample = nn.Sequential(
            nn.Conv2D(in_channels=self.len_list[IND-1], out_channels=self.len_list[IND+2], kernel_size=1, stride=stride, padding=0, bias_attr=False),
