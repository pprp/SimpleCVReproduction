...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, cfg=None, num_classes=10):
        ...
        count = 0
        self.layer1 = self._make_layer(block, 16, num_blocks[0], cfg[:num_blocks[0]], stride=1)
        count += num_blocks[0]
        self.layer2 = self._make_layer(block, 32, num_blocks[1], cfg[count:count+num_blocks[1]], stride=2)
        count += num_blocks[1]
        self.layer3 = self._make_layer(block, 64, num_blocks[2], cfg[count:count+num_blocks[2]], stride=2)
    ...
    def _make_layer(self, block, planes, num_blocks, cfg, stride):
        ...
        for idx, stride in enumerate(strides):
            layers.append(block(self.in_planes, planes, cfg[idx], stride))
